...
class TestSubnet(base.TestCase):
    ...
    def test_Subnet23(self):
        ...
        self.assertEquals("192.168.2.3/23", subnet2.address(0))
        self.assertEquals("192.168.3.254", subnet2.gateway)
        self.assertEquals("192.168.3.1/23", subnet2.address(254))
        self.assertRaises(ValueError, subnet2.address, 511)
        print subnet2.host_ip_range
        self.assertEquals("192.168.3.251/23", subnet2.address(-1))

    def test_Subnet24(self):
        network = netaddr.IPNetwork("192.168.2.0/24")
        network_broadcast = "192.168.2.255"
        network_prefix = "24"
        subnet24 = objects.Subnet(network)
        self.assertEquals("192.168.2.251/24", subnet24.address(-1))

    def test_Subnet_range(self):
        network = netaddr.IPNetwork("192.168.2.0/23")
        kwargs = {"host_ip_range":"192.168.2.10, 192.168.2.100"}
