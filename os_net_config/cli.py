...
def main(argv=sys.argv):
    ...
    iface_array = []
    subnets_array = []
    ooo_data_files = ['controller.yaml',
                      'compute.yaml',
                      'ceph.yaml',
                      'object.yaml',
                      'volume.yaml']
    ...
    if os.path.exists(opts.config_file):
        with open(opts.config_file) as cf:
            config_raw = cf.read()
            config_processed = utils.replace_tokens(config_raw, ip_index)
            iface_array = yaml.load(config_processed).get("network_config")
    ...
    if not isinstance(subnets_array, list):
        logger.warning('No subnets defined in config: %s' % opts.config_file)
        subnets_array = []
    ...
    for iface_json in iface_array:
        logger.debug("iface_json: %s" % iface_json)
