...
class Subnet(object):
    ...
    def __init__(self, ip_netmask, name="", **kwargs):
        ...
        :param host_ip_range: Default excludes first 3 and last 3 host IPs.
                              Note that this range should be different for
                              each group of hosts being deployed by TripleO.
        """
        self.ip_netmask = str(ip_netmask)
        self.network = netaddr.IPNetwork(ip_netmask)
        self.gateway = str(self.network[-2])
        self.host_ip_range = netaddr.IPRange(str(self.network[3]),
                                             str(self.network[-5]))
        logger.debug('Name: %s' % name)
        if name:
            self.name = name
        else:
            # construct network name from ip_netmask
        ...
        if "host_ip_range" in args:
            begin, end = args["host_ip_range"].split(',')
            self.host_ip_range = netaddr.IPRange(begin.strip(), end.strip())
        for key, value in kwargs.iteritems():
            self.key = value
    ...
    def address(self, ip_index):
        """Returns the <index>th address in the host_ip_range, or None if
           out of range.

        :param ip_index: the index position of the address to retrieve
        :returns: an IPv4Address object representing the IP address.
        """
        try:
            __retval = str(self.host_ip_range[ip_index]) + '/' + \
